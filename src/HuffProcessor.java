...
public class HuffProcessor {
	...
	private HuffNode readTreeHeader(BitInputStream in) {
		...
		if (bit == -1) throw new HuffException("illegal bit value -1");
		       ...
		       if (bits == -1) throw new HuffException("bad input, no PSEUDO_EOF");
		    	   ...
		    	   if(current.myLeft == null && current.myRight == null) {
		    		   ...
		    		   if(current.myValue == PSEUDO_EOF) break;
		    			   ...
		    			   out.writeBits(current.myValue, BITS_PER_WORD);
		    			   current = root;
		    		   }  
		    	   } 
		       }
	...
	private void encodingHelper(HuffNode root, String encodedPath, String [] encodings) {
		...
		encodingHelper(root.myLeft, encodedPath+"0", encodings);
		encodingHelper(root.myRight, encodedPath+"1", encodings);
	}
